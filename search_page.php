...
if(isset($_POST['add_to_wishlist'])){
    ...
    if(mysqli_num_rows($check_wishlist_numbers) > 0){
        $message[] = 'Ya esta en wishlist';
    }elseif(mysqli_num_rows($check_cart_numbers) > 0){
        $message[] = 'Ya esta en carro';
    }else{
...
<section class="heading">
    ...
    <p> <a href="home.php">Inicio</a> / search </p>
</section>

<section class="search-form">
    <form action="" method="POST">
        <input type="text" class="box" placeholder="Buscar Productos." name="search_box">
   ...
   <div class="box-container">
      ...
      <?php
        if(isset($_POST['search_btn'])){
         ...
         $select_products = mysqli_query($conn, "SELECT * FROM `prodcutos` WHERE nombre LIKE '%{$search_box}%'") or die('query failed');
      ...
      <form action="" method="POST" class="box">
         ...
         <input type="submit" value="añadir wishlist" name="add_to_wishlist" class="option-btn">
         <input type="submit" value="añador al carri" name="add_to_cart" class="btn">
      </form>
      <?php
         }
            }else{
                echo '<p class="empty">No resultado</p>';
            }
        }else{
            echo '<p class="empty">Buscal algo</p>';
        }
      ?>
